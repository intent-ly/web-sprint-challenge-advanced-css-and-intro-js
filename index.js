...
born in and died in 20th century (1900-2000) example born in 1901 and died in 1959 - included / born in 1889 and died in 1925 not included - should return ["Salvador Dali", "Frida Kahlo"]*/
...
function get20s(arr){
  ...
  var ageInt;
    for(let i=0; i<arr.length; i++){
      ageString= arr[i].years.split("-");
      ageInt = parseInt(ageString, 10);
      if(ageInt >=1900 && ageInt<=2000){
        getTwenty[q]=arr[i].name;
        q=q+1;
      }
    }
  return getTwenty;
...
bio: Erick has been involved with photography from the years 2018 to 2020. He now spends most of his time programming in Southern California. 
}
At the end, this function should return the new array with information added"*/

function addArtist(customObject){
    artists.push(customObject);
    console.log(artists);
  }
...
function lotsOfArt(artists){
  ...
  for (let i=0; i < artists.length; i++){
      if (artists[i].paintings > 100){
          paintingsAmount.push(artists[i].name);
      }
  }
